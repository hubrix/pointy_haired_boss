...
Inspired by [Ponytail](https://github.com/DietrichGebert/ponytail)'s persistent, adjustable guidance for coding agents. The repository contains a runnable local checker, shared contracts, research, and parser/host experiments. Contextual editing, Unicode cleanup, readability/Chicago checks, and installable host plugins remain under development.
...
`check` is read-only. Its default required scope covers prose boundaries and exact phrase bans; all four house bans produce candidates for contextual review. Reports show incomplete and omitted checks. Exit `0` does not certify full editorial compliance. See the [checker guide](docs/CHECKER.md) for stdin, selections, profiles, exclusions, trusted directives, and exit codes.
