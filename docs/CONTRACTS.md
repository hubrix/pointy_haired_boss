...
There are now ten generated schemas, including readability metrics, Chicago subset metadata, Unicode inventory, and cleanup plans. The [style guide](STYLE.md) defines metric units, unavailable states, source editions, and candidate scope. Rule READ-01 version 1.1.0 is a local heuristic metric with no fix; contextual comprehension remains outside it. CMO-01–06 version 1.1.0 retain contextual family coverage. `unicode` configuration adds `bom: remove|preserve` and a restricted `remove` array. The [Unicode guide](UNICODE.md) documents inventory and cleanup plans. Shape validation alone cannot authorize a write: the cleaner regenerates and compares the entire plan against current source and policy.

The suite includes 38 contract tests, 25 parser-spike tests, 40 checker tests, 37 Unicode/cleanup tests, and 35 readability/Chicago tests. The exact declared minimum of Node 22.18 still needs a compatibility run. Contract tests cover invalid/inactive settings, profile precedence, explicit ban overrides, fidelity rules, Unicode source positions, suppressions, protected conflicts, fix preconditions, and coverage. Style tests cover known metric counts, sample limits, source mapping, preservation, edition-specific candidates, overrides, and CLI results. Cleanup tests cover preservation, policy opt-ins, idempotence, stale/forged plans, file permissions/metadata, and network-disabled execution. The tracker records the tested runtime versions and current results.

[project.org](../project.org) owns current verification and remaining work. PHB-021 added parsing and local detectors; PHB-022 added Unicode inventory and cleanup; PHB-023 adds readability and the declared Chicago subset. PHB-024 still needs contextual review, and PHB-025 needs general patch application. Metadata and passing contract tests do not establish those later capabilities.
